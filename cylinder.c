...
int find_tow_lines_intersection(const t_tuple *l1o, const t_tuple *v1, const t_ray *r, float *ty)
{
    t_tuple l1v;
    t_tuple l2v;
    t_tuple l2_to_l1o;
	...
	*ty = 10;
    ...
    if (l1v.x == 0 && l2v.y == 0)
    {
        // Lines are parallel along the y-axis
        return 0;
    }
...
t_hit_info	ray_cylinder_intersect(const t_ray *r, const t_cylndr *c)
{
	...
	if (fabs(cx_dist) > c->rad)
		return (inf);
	// cy_v = cros_multiplication(&r->direction, &cx_v);
	// cy_v = vec_norm(&cy_v);
	// cy_dist = dot_multiplication(&cc_ro_v, &cy_v);
	// if (fabs(cy_dist) > c->height / 2)
	// 	return (inf);
	p0_plus_t_mul_v(&cy_p, &c->cnt, &cx_v, cx_dist);
	if (!find_tow_lines_intersection(&cy_p, &c->vec, r, &cy_dist))
	{
		//ray direct on cylinder vector
		cy_dist = dot_multiplication(&cc_ro_v, &c->vec);
		cy_dist = 3;
		// find_tow_lines_intersection(&c->cnt, &c->vec, r, &cy_dist);
	}
	if (fabs(cy_dist) > c->height / 2)
		return (inf);
	inf.hit_or_not = 1;
